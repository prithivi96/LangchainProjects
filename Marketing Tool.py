...
def getLLMResponse(query, age_option, tasktype_option):
    ...
    response = llm.invoke(new_prompt_template.format(template_userInput=query, template_ageoption=age_option,
                                                     template_tasktype_option=tasktype_option))
    print(response)
...
task_type = st.selectbox(
    ...
    options=('Write a sales copy', 'Create a tweet', 'Write a product description'),
    key=1)
...
number_of_words = st.slider(label="Words Count", min_value=1, max_value=200, step=25)

submit = st.button("Submit")

if submit:
    st.write(getLLMResponse(query=form_input, tasktype_option=task_type,
